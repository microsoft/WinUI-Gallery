...
`code` carries no such guarantee yet. A C# token usually sits where deleting it would not leave valid code — a call argument such as `ColorHelper.FromArgb($(Color))`, or an enum member suffix such as `PickerLocationId.$(Location)` — so those samples publish the token verbatim, and a consumer should treat `code` as possibly still templated.
